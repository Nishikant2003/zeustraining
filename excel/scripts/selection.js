class Selection {
  ...
  isRowSelected(row) {
    if (!this.currentSelection) return false;
    const { startRow, endRow } = this.currentSelection;
    return row >= startRow && row <= endRow;
}

isColSelected(col) {
    if (!this.currentSelection) return false;
    const { startCol, endCol } = this.currentSelection;
...
drawSelectionOutline() {
    ...
    const ctx = grid.ctx;
    let { startRow, startCol, endRow, endCol } = currentSelection;

    // Prevent outline from covering headers
